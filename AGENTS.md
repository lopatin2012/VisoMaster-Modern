...
- To localize a new string: wrap it with `i18n.tr(...)` (or `set_widget_text`) and add an entry to `TRANSLATIONS` in `i18n.py` (`ru` inline, `zh` in `app/helpers/i18n_zh.py`). `README.ru.md` and `README.zh.md` mirror `README.md`.
