...
VisoMaster: PySide6 desktop app for AI face swapping and editing (images, video, webcam). No test suite, no lint/typecheck config, no CI.

## Run / setup
- Target Python 3.10.13 in a conda env named `visomaster`; install `requirements_cu124.txt` (or `requirements_cu118.txt`).
...
## Verify
- No tests, no headless mode. Verify changes by running the app (`python main.py`). Do not invent test/lint commands.
